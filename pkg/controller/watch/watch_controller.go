...
type ReconcileWatch struct {
	...
	mgr         manager.Manager
	controllers map[string]*apicontroller.APIReconciler
	mtx         sync.Mutex
}
...
func (r *ReconcileWatch) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	reqLogger.Info("***********************************")
	reqLogger.Info(instance.Spec.ServiceName)
	reqLogger.Info(instance.Spec.GVK().String())
	reqLogger.Info(fmt.Sprintf("%v", instance.Spec))

	r.mtx.Lock()
	defer r.mtx.Unlock()

	if r.controllers == nil {
		r.controllers = map[string]*apicontroller.APIReconciler{}
	}

	name := instance.Spec.GVK().String()
	_, ok := r.controllers[name]
	if !ok {
		controller, err := apicontroller.Add(r.mgr, instance)
		if err != nil {
			return reconcile.Result{}, err
		}
